...
formatTable = {
    "R8G8_UNORM": [2, 1, 1, 1],
    "R4_G4_UNORM": [1, 1, 1, 1],  # probably incorrect
    ...
    for depthLevel in range(numDepth):
        for arrayLevel in range(texture["arrayCount"]):
            ...
            for mipLevel in range(texture["mipCount"]):
                ...
                size = DIV_ROUND_UP(width, blkWidth) * \
                    DIV_ROUND_UP(height, blkHeight) * bpp
                ...
                alignedData = bytearray(
                    round_up(surfaceSize, dataAlignment) - surfaceSize)
                ...
                msize = int(
                    (mipOffsets[0] + len(imageData) - mipOffsets[mipLevel]) / texture["arrayCount"])

                data_ = subArray(imageData, arrayOffset +
                                 mipOffsets[mipLevel], msize)
                try:
                    pitch = round_up(width__ * bpp, 64)
                    surfaceSize += pitch * \
                        round_up(height__, max(
                            1, blockHeight >> blockHeightShift) * 8)
                    result = deswizzle(width, height, depth, blkWidth, blkHeight, blkDepth, target, bpp, tileMode, max(
                        0, blockHeightLog2 - blockHeightShift), data_)
            ...
            for mipLevel in range(texture["mipCount"]):
                ...
                try:
                    ...
                    surfaceSize += pitch * \
                        round_up(height__, max(
                            1, blockHeight >> blockHeightShift) * 8)
                    result = swizzle(width, height, depth, blkWidth, blkHeight, blkDepth, target, bpp, tileMode, max(
                        0, blockHeightLog2 - blockHeightShift), data_)
